...
const uint8_t BUTTON1_PIN = 32; // Show all networks
const uint8_t BUTTON2_PIN = 33; // Show open networks
const uint8_t BUTTON3_PIN = 25; // Show web-access networks
const uint8_t BUTTON4_PIN = 26; // Show vulnerable networks
const uint8_t LED_PIN = 2;      // Status LED
...
enum ViewMode { VIEW_ALL, VIEW_OPEN, VIEW_WEB, VIEW_VULN };
ViewMode currentView = VIEW_ALL;
bool isScanning = false;

std::vector<WiFiNetwork> openNetworks;
std::vector<WiFiNetwork> webNetworks;
...
void renderCurrentView() {
  displayTopBar(isScanning);
  switch (currentView) {
  case VIEW_ALL:
    displayList(finalNetworks);
    break;
  case VIEW_OPEN:
    displayList(openNetworks);
    break;
  case VIEW_WEB:
    displayList(webNetworks);
    break;
  case VIEW_VULN:
    displayList(vulnerableNetworks);
    break;
  }
...
void handleButtons() {
  ...
  if (digitalRead(BUTTON1_PIN) == LOW)
    ...
    currentView = VIEW_OPEN;
  else if (digitalRead(BUTTON3_PIN) == LOW)
    currentView = VIEW_WEB;
  else if (digitalRead(BUTTON4_PIN) == LOW)
    currentView = VIEW_VULN;
...
void loop() {
  ...
  if (millis() - lastScan > scanInterval) {
    ...
    vulnerableNetworks = getVulnerableWifiNetworks(closed);
    webNetworks = getValidatedOpenWifiNetworks(openNetworks);
    finalNetworks =
        mergeAndOrderWifiNetworks(closed, openNetworks, webNetworks, vulnerableNetworks);
    isScanning = false;
    digitalWrite(LED_PIN, LOW);
    renderCurrentView();
  }
